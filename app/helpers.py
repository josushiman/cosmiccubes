...
class ReactAdmin():
    ...
    async def get_list(cls, model: Model, commons: dict, kwargs_raw: dict) -> tuple:
        ...
        if 'id' in kwargs_raw and type(kwargs_raw['id']) is list: return await cls.get_many(model, kwargs_raw['id'])

        # Otherwise process the kwargs, limit, and order values
        kwargs = await cls.process_raw_kwargs(model, kwargs_raw)
    ...
    @classmethod
    async def get_many(cls, model: Model, ids: list[UUID]) -> tuple:
        results = []
        # Get each entity from the id's provided in the list.
        for entity_id in ids:
            db_entity = await cls.get_one(model, entity_id)
    ...
    @classmethod
    async def process_raw_kwargs(cls, model: Model, kwargs_raw: dict):
