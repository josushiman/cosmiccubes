...
from app.db.schemas import (
    ...
    YnabCategories_Pydantic,
    YnabMonthSummaries_Pydantic,
    YnabPayees_Pydantic,
...
class ReactAdmin:
    ...
    async def get_list(cls, resource: str, commons: dict, kwargs_raw: dict) -> tuple:
        ...
        entities = await cls.get_entity_list_data(
            entity_model,
            resource,
            limit,
            commons["_start"],
        ...
        row_count = await cls.get_entity_list_count(
            entity_model, kwargs if kwargs != {} else None
        )
    ...
    async def create(cls, resource: str, resp_body: dict):
        ...
        try:
            ...
            raise HTTPException(
                status_code=400
            ) from e_attr  # May have missed adding '_id' at the end of any FK relations in the json. e.g. type = type_id
    ...
    @classmethod
    async def get_entity_list_data(
        cls,
        entity_model: Model,
        resource: str,
        limit: int,
            ...
            if order_by is None:
                if filter:
                    return await entity_schema.from_queryset(
                        entity_model.filter(**filter).limit(limit).offset(offset)
                    )
                return await entity_schema.from_queryset(
                ...
                return await entity_schema.from_queryset(
                    entity_model.all().limit(limit).offset(offset).order_by(order_by)
                )
    ...
    @classmethod
    async def get_entity_list_count(
        cls, entity_model: Model, filter: dict = None
    ) -> int:
    ...
    async def update(cls, resource: str, resp_body: dict, _id: UUID):
        ...
        try:
            ...
            logging.info(
                "Incorrect fields being passed to the model.", exc_info=e_field
            )
